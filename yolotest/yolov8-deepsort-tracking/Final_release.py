...
def generate_frame_intervals(frames, range_size):
    intervals = []
    for frame in frames:
        intervals.append(list(range(frame - range_size, frame + range_size + 1)))
    return intervals

def calculate_recall_and_precision(true_frames, predicted_intervals):
    true_frames_set = set(true_frames)
    predicted_frames_set = set([frame for interval in predicted_intervals for frame in interval])
    
    true_positives = len(true_frames_set & predicted_frames_set)
    false_negatives = len(true_frames_set - predicted_frames_set)
    false_positives = len(predicted_frames_set - true_frames_set)
...
if __name__ == "__main__":
    ...
    range_size = 300
    true_labels_list = [        np.arange(5890,6336).tolist(),
        np.arange(1086, 1115).tolist(),
        np.arange(10680, 10752).tolist(),
        np.arange(6304, 6386).tolist(),
        np.arange(35091,25099).tolist(),
        np.arange(28800,29216).tolist(),
        np.arange(17409,17524).tolist(),
        np.arange(11551,11796).tolist(),
        np.arange(27902,28171).tolist(),
        np.arange(14198,14580).tolist(),
    ]
    
    # 所有真实帧的集合
    true_frames_set = set([frame for sublist in true_labels_list for frame in sublist])
    results = []
    for true_frames, predicted_samples in zip(true_labels_list, query_info):
        predicted_intervals = generate_frame_intervals([frame for frame, _ in predicted_samples], 300)
        recall, precision = calculate_recall_and_precision(true_frames, predicted_intervals)
        results.append((recall, precision))

    # 打印每个样本的召回率和准确率
    for i, (recall, precision) in enumerate(results):
        print(f"Sample {i + 1}:")
